...
#     def _generate_schedule(self, input_data: ScheduleInput) -> List[ScheduleAssignment]:
#         """Generate schedule without extending periods or creating virtual faculty."""
#         schedule = []
...
#         # Track unassigned slots with reasons
#         for day in VALID_DAYS:
#             for slot_label in self.time_slot_labels:
#                 start_time, end_time = slot_label.split('-')
#                 slot = TimeSlot(day=day, startTime=start_time, endTime=end_time)
#                 slot_key = (day, start_time, end_time)
                
#                 if check_break_conflict(slot, input_data.break_):
#                     continue
#                     if not any(a.day == day and a.startTime == start_time and a.endTime == end_time for a in schedule):
#                         slot_duration = self._get_slot_duration(slot)
#                         reason = None
                    
#                         # Check if any subject matches duration
#                         matching_subjects = [s for s in input_data.subjects if s.time == slot_duration]
#                         if not matching_subjects:
#                             reason = "No subjects available for this duration"
#                         else:
#                             # Check if any faculty is available
#                             has_available_faculty = False
#                             for subject in matching_subjects:
#                                 if self.subject_counts.get(subject.name, 0) < subject.no_of_classes_per_week:
#                                     for faculty in subject.faculty:
#                                         if self._is_faculty_available_for_slot(faculty, slot) and self._is_slot_available(faculty.id, self.single_room_id, slot):
#                                             has_available_faculty = True
#                                             break
#                                     if has_available_faculty:
#                                         break
                        
#                             if not has_available_faculty:
#                                 # Check if due to period limit
#                                 all_periods_full = all(self.subject_counts.get(s.name, 0) >= s.no_of_classes_per_week for s in matching_subjects)
#                                 reason = "No periods available" if all_periods_full else "No faculty available"
                    
#                             if reason:
#                                 self.unassigned_reasons[slot_key] = reason
        
#         return schedule

#     def generate_schedule(self, input_data: ScheduleInput, use_ga: bool = False) -> Dict[str, Any]:
#         """Generate a weekly schedule without extending periods."""
#         try:
...
#         else:
#             schedule = self._generate_schedule(input_data)
...
#         fitness = 1.0 - (unassigned_slots / total_available_slots) if total_available_slots > 0 else 0
...
#             "subject_coverage": subject_coverage,
#             "guaranteed_100_percent": False
#         }

from typing import List, Dict, Any, Tuple, Optional
from collections import defaultdict
from model import ScheduleInput, ScheduleAssignment, TimeSlot, TimeSlot, Break, Subject, Faculty
from utils import check_time_conflict, check_break_conflict, time_to_minutes, minutes_to_time, VALID_DAYS, generate_time_slots, generate_weekly_time_slots, calculate_preference_score
import random
from datetime import datetime
import logging
...
class SchedulerService:
    ...
    def _guaranteed_100_percent_fill(self, schedule: List[ScheduleAssignment], input_data: ScheduleInput) -> List[ScheduleAssignment]:
        """Ensure 100% slot utilization using only existing subjects and faculty."""
        ...
        subject_weights = {s.name: s.no_of_classes_per_week / total_required if total_required > 0 else 1/len(input_data.subjects) for s in input_data.subjects}
        
        # Sort available slots by day and time
        available_slots.sort(key=lambda s: (VALID_DAYS.index(s.day), time_to_minutes(s.startTime)))
        
        # Phase 1: Fill remaining slots with existing subjects
        ...
        for slot in remaining_slots:
            ...
            # Select subject based on proportional weights
            subject = random.choices(
                input_data.subjects,
                weights=[subject_weights[s.name] for s in input_data.subjects],
                k=1
            )[0]
        ...
        # Phase 2: Handle any remaining unassigned slots (rare, due to constraints)
        remaining_slots = [slot for slot in available_slots if (slot.day, slot.startTime, slot.endTime) not in assigned_slots]
        if remaining_slots:
            logger.warning(f"Phase 2: {len(remaining_slots)} slots unassigned due to constraints. Attempting reassignment...")
            for slot in remaining_slots:
                slot_key = (slot.day, slot.startTime, slot.endTime)
                slot_duration = self._get_slot_duration(slot)
                if slot_duration <= 0:
                ...
                # Try all subjects with matching duration
                for subject in input_data.subjects:
                    if subject.time != slot_duration:
                        continue
                    for faculty in subject.faculty:
                        if self._is_faculty_available_for_slot(faculty, slot) and self._is_slot_available(faculty.id, self.single_room_id, slot):
                            ...
                            self._add_assignment(assignment)
                            assigned_slots.add(slot_key)
                            logger.debug(f"Phase 2: Assigned {subject.name} to {slot.day} {slot.startTime}-{slot.endTime}")
                            break
                    if slot_key not in assigned_slots:
                        self.unassigned_reasons[slot_key] = f"No available faculty for any subject at {slot.day} {slot.startTime}-{slot.endTime}"
        
        logger.info(f"Final assignment count: {len(new_assignments)}")
        return new_assignments

    def generate_schedule(self, input_data: ScheduleInput, use_ga: bool = False) -> Dict[str, Any]:
        """Generate a weekly schedule with 100% slot utilization using only existing subjects and faculty."""
        try:
            self._validate_input(input_data)
        except ValueError as e:
            logger.error(f"Input validation failed: {e}")
        ...
        if use_ga:
            ...
            # Phase 2: Guarantee 100% utilization
