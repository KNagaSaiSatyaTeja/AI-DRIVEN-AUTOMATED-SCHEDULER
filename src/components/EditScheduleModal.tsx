...
export function EditScheduleModal({
  ...
  const onSubmit = (data: ScheduleEntry) => {
    if (isNewEntry) {
      const isClash = schedule.some(
        (e) =>
          e.day === data.day && e.time === data.time && e.room === data.room
      );
      if (isClash) {
        toast({
  ...
  const availableFaculty = selectedSubject
    ? faculty.filter((f) => selectedSubject.facultyIds.includes(f._id))
    : [];
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {isSlotEditable ? "Add New Schedule Entry" : "Edit Schedule Entry"}
          </DialogTitle>
          <DialogDescription>
            {isSlotEditable
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    onValueChange={field.onChange}
                    value={field.value}
                    disabled={isBreak || !selectedSubjectName}
                  >
                    <FormControl>
                      <SelectTrigger>
