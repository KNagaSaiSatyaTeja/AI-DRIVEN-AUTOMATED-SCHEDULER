...
export function Timetable() {
  ...
  // Function to check if a time slot is a break
  const isBreakTime = (slot: string) => {
    const breakTimes = [
      "10:00-10:15", "10:15-10:30", // Morning break
      "12:00-12:45", "12:45-13:30", // Lunch break
      "15:00-15:15", "15:15-15:30", // Afternoon break
    ];
    return breakTimes.includes(slot);
  };

  // Function to get break label
  const getBreakLabel = (slot: string) => {
    const breakLabels: { [key: string]: string } = {
  ...
  const openEdit = (day: string, timeSlot: string) => {
    // Don't allow editing break times
  ...
  const exportToCSV = () => {
    ...
    let csv = `TIMETABLE REPORT\n`;
    csv += `Room: ${timetable.name}\n`;
    csv += `Generated on: ${currentDate}\n`;
    csv += `\n`;
    
    // Table 1: Subject and Timetable
    csv += `TABLE 1: SUBJECT TIMETABLE\n`;
    csv += `Day,${timeSlots.join(",")}\n`;
    ...
    days.forEach((day) => {
      ...
      timeSlots.forEach((slot) => {
        if (isBreakTime(slot)) {
          ...
          row.push(match ? match.subject_name : "Free Period");
        }
      });
    ...
    // Table 2: Faculty and Subject Assignment
    csv += `TABLE 2: FACULTY ASSIGNMENT\n`;
    csv += `Day,Time Slot,Subject,Faculty,Room\n`;
    ...
    days.forEach((day) => {
      timeSlots.forEach((slot) => {
        if (!isBreakTime(slot)) {
          ...
          if (match) {
            csv += `"${day}","${slot}","${match.subject_name}","${match.faculty_name}","${timetable.name}"\n`;
          } else {
            csv += `"${day}","${slot}","Free Period","N/A","${timetable.name}"\n`;
          }
        } else {
          csv += `"${day}","${slot}","${getBreakLabel(slot)}","N/A","${timetable.name}"\n`;
        }
      });
    });
    ...
    csv += `\n\n`;
    csv += `SUMMARY\n`;
    csv += `Total Teaching Periods,${timetable.schedule?.length || 0}\n`;
    csv += `Total Break Periods,${timeSlots.filter(slot => isBreakTime(slot)).length * days.length}\n`;
    csv += `Room Utilization,${Math.round((timetable.schedule?.length || 0) / (timeSlots.filter(slot => !isBreakTime(slot)).length * days.length) * 100)}%\n`;

    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `Timetable_Report_${timetable?.name}_${currentDate.replace(/\//g, '-')}.csv`;
    link.click();
  };
    ...
    <Card>
      <CardHeader className="flex flex-row justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={exportToCSV} disabled={!timetable}>
            <Download className="w-4 h-4 mr-2" /> CSV Report
          </Button>
        </div>
      </CardHeader>
